...
def backend() -> str:
    return "postgres" if (os.environ.get("LOCI_DATABASE_URL") or os.environ.get("LOCI_DB_HOST")) else "sqlite"


def _pg_dsn() -> str:
    if url := os.environ.get("LOCI_DATABASE_URL"):
        return url
    secret = json.loads(os.environ["LOCI_DB_SECRET"])  # RDS-managed master secret: {"username","password"}
    return (f"postgresql://{secret['username']}:{secret['password']}@{os.environ['LOCI_DB_HOST']}:"
...
def connect():
    if backend() == "postgres":
        ...
        return psycopg.connect(_pg_dsn(), row_factory=dict_row, autocommit=True, connect_timeout=5)
